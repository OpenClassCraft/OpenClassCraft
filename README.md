...
2. Place a habitat or farm token for a rabbit, deer, fox, squirrel, duck, cow, chicken, Indian bullfrog, smooth-coated otter, wild boar, Himalayan tahr, or olive ridley turtle. New terrain also spawns a bounded biome-appropriate mix naturally.
...
All fourteen species use lightweight rigged models, visible eyes, species textures, unique real-world-relative scales, distinct calls, and blended idle, walk, run, sit, graze, sleep, drink, alert, climb, and swim animation ranges. Animals build hunger, thirst, and fatigue; seek nearby food and water; graze, drink, and sleep; and form flocks, herds, or family groups where appropriate. Rabbits and squirrels freeze before bounding away, squirrels climb nearby trees when pursued, deer watch from farther off, frogs leap and swim, otters investigate cautiously, boar keep with their sounder, tahr avoid exposed drops, and turtles withdraw from close approaches. Stationary animals use smoothly limited head and eye-line tracking to observe nearby players without rotating their whole body; full-body turns remain reserved for walking, fleeing, following, and hunting. Wild foxes stalk, chase, and complete bounded prey encounters while prey use evasive paths. Dogs greet players socially and cats observe before approaching. Movement accelerates and turns smoothly, uses full-width wall and cliff probes, and recovers from stalled routes without unrestricted teleporting.

Natural spawning leaves a 22-node clear area around every player, pauses at five animals within 56 nodes, caps smaller habitat clusters and individual species, and gives each 48-node habitat cell a 90-second cooldown. These defaults can be changed under **OpenClassCraft wildlife population** in Settings. Tokens remain deliberate educator tools: manually placed animals and owned pets are never removed by the population controller.
...
Wild populations follow those habitats: frogs and otters join ducks around freshwater; boar and frogs occupy monsoon forest; tahr use montane and alpine ground; and olive ridley turtles use mangrove coast and shallow reef water. Farm animals and pets remain educator-placed rather than spawning as wildlife. Habitat ambience crossfades among forest, river, coast, and wind layers, while deterministic local rain, snow, and dust weather adds bounded particles and matching audio. Existing explored map chunks are not regenerated, so create a new world to see the complete biome system.
...
| [`util/generate_ecology_media.py`](util/generate_ecology_media.py) | Reproducibly generates species textures, portraits, calls, interaction sounds, and habitat ambience. |
| [`util/check_openclasscraft_living_world.py`](util/check_openclasscraft_living_world.py) | Validates the complete model, animation, texture, sound, and font media set. |
...
| [`games/luanti_edu/mods/openclasscraft_world/`](games/luanti_edu/mods/openclasscraft_world/) | Sky, biome weather, layered ambience, music, vegetation, and classroom world styling. |
